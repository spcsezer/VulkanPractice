...
void ValidationLayers::gValidationLayers()
{
	setupDebugMessenger();
}

void ValidationLayers::cleanUp()
{
	if (gVulkanContext.enableValidationLayers) {
		DestroyDebugUtilsMessengerEXT(gVulkanContext.instance, debugMessenger, nullptr);
	}
}
...
bool ValidationLayers::checkValidationLayerSupport()
{
	...
	std::vector<VkLayerProperties> availableProperties(layerCount);
	vkEnumerateInstanceLayerProperties(&layerCount, availableProperties.data());
...
{
	if (!gVulkanContext.enableValidationLayers) return;

	VkDebugUtilsMessengerCreateInfoEXT createInfo;
	populateDebugMessengerCreateInfo(createInfo);

	if (CreateDebugUtilsMessengerEXT(gVulkanContext.instance, &createInfo, nullptr, &debugMessenger) != VK_SUCCESS) {
