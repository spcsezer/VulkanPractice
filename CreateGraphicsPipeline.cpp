...
{
	vkDestroyPipeline(gVulkanContext.device, graphicsPipeline, nullptr);
...
void CreateGraphicsPipeline::createGraphicsPipeline(const char* vertexShaderFile, const char* fragmentShaderFile, VkPipelineLayout& pipelineLayout, VkPipeline& graphicsPipeline, VkDescriptorSetLayout& descriptorSetLayout)
{
	...
	viewport.x = 0.0f;
	viewport.y = 0.0f;
	viewport.width = (float)gVulkanContext.swapchainExtent.width;
	viewport.height = (float)gVulkanContext.swapchainExtent.height;
	...
	multisampling.rasterizationSamples = gVulkanContext.msaaSamples;
	...
	if (vkCreatePipelineLayout(gVulkanContext.device, &pipelineLayoutInfo, nullptr, &pipelineLayout) != VK_SUCCESS)
	{
	...
	pipelineInfo.renderPass = gVulkanContext.renderPass;
	pipelineInfo.subpass = 0;
	pipelineInfo.basePipelineHandle = VK_NULL_HANDLE;
	pipelineInfo.basePipelineIndex = -1;

	if (vkCreateGraphicsPipelines(gVulkanContext.device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline) != VK_SUCCESS)
	{
		throw std::runtime_error("!-- Failed to create graphics pipeline --!");
	}

	vkDestroyShaderModule(gVulkanContext.device, vertShaderModule, nullptr);
	vkDestroyShaderModule(gVulkanContext.device, fragShaderModule, nullptr);
}

std::vector<char> CreateGraphicsPipeline::readFile(const std::string& fileName)
{
...
VkShaderModule CreateGraphicsPipeline::createShaderModule(const std::vector<char>& code)
{
